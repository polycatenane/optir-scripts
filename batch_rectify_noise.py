#!/usr/bin/env python3
"""Batch version of rectify_noise for a directory of TIFF images.

For each TIFF file in an input directory, this script:

1. Multiplies it element-wise with a reference TIFF (float32).
2. Computes the mean value of the resulting product image.
3. Subtracts this mean from every pixel of the input image.
4. Writes the rectified image as a new float32 TIFF with ``_rectified`` suffix.
5. Moves the original input TIFF into a separate directory.

Usage
-----

    batch_rectify_noise.py INPUT_DIR reference.tif [--moved-dir ORIGINALS]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "reference",
        help="Reference TIFF file (float32) multiplied with each input image.",
    )
    parser.add_argument(
        "--moved-dir",
        "-m",
...
def main() -> None:
    ...
    for inp_path in tiff_paths:
        ...
        if img.shape != ref_img.shape:
            print(
                f"Skipping {inp_path}: shape mismatch input={img.shape}, "
                f"reference={ref_img.shape}",
                file=sys.stderr,
            )
            continue
        ...
        print(
            ...
            f"moved original to {dest_path} (mean_product={mean_val})"
        )
